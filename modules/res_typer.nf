process res_typer {
    ...
    input:
    tuple val(pair_id), file(arg_fullgenes), file(arg_genes), file(resfinder_fullgenes), file(resfinder_genes), file(RES_fullgenes), file(RES_genes), file(RES_consensus)
...
process srst2_for_res_typing {
    ...
    tuple val(pair_id), file("*${pair_id}__fullgenes__*__results.txt"), file("*${pair_id}__genes__*__results.txt"), emit: genes_files
...
process freebayes {
    ...
    output:
    tuple val(pair_id), file("${pair_id}_*_consensus_seq.fna")
